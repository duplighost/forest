...
export class Critters {
  ...
  _newTarget(b, center) {
    ...
    // sometimes drift down to land on a flower / blade of grass
    b.landing = ground > WORLD.waterLevel + 0.3 && Math.random() < 0.4;
    const y = b.landing ? ground + 0.18 : ground + 1.2 + Math.random() * 4;
    b.target.set(x, y, z);
  ...
  update(dt, center) {
    for (const b of this.list) {
      ...
      if (reached && b.landing) { b.rest = 2.5 + Math.random() * 4; continue; }
      if (b.retarget <= 0 || reached) this._newTarget(b, center);
      ...
      b.g.rotation.set(b.landing ? 0.3 : 0, yaw, Math.sin(b.phase * 3) * 0.15);
